...
contains
	...
	subroutine writeStepXYZ(iou)
		...
		do k=1,size(atoms)
			!write(iou,'(1A2,1X,3F25.15)') types(atoms(k)%t)%atom_name, [( convert(atoms(k)%v(i),'m/s','A/ps') , i=1,3 )]
			...
			write(iou,'(1A2,1X,3F25.15)') types(atoms(k)%t)%atom_name, [(convert(atoms(k)%f(i), 'N', 'eV/A'), i=1,3)]
