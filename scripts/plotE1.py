...
def plotFlux(ax,t,q):
	...
	ax.plot(t[:]/1000,savgol_filter(q[:],sfp,3), color='darkviolet', linewidth=1.5)
...
def plotHC(ax,t,H,C):
	...
	ax.plot(t[:]/1000,savgol_filter(H[:],sfp,3), color='darkred', linewidth=1.5)
	ax.plot(t[:]/1000,savgol_filter(C[:],sfp,3), color='darkblue', linewidth=1.5)
	ax.set_xlabel('Time $t$ [ns]')
...
def plotG(ax,Ti):
	...
	a = 5.4
	z = pl.linspace(0,15*a,T.shape[1])
...
def doFigure(fnE,fnT,box):
	...
	plotHC(axHC,tT,TH,TC)
	
	axQ = fig.add_subplot(224)
	heatRate = plotFlux(axQ,tE,q)
...
a = 5.4
f, T, k2 = doFigure('step1.energies','step1.temps',a*pl.array([5,5,15]))
A = 5.4*5*5.4*5
flux = pl.mean(f[:]/0.1/A)
dz = 15*5.4/2
